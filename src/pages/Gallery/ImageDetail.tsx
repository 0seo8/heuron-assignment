...
const ImageDetailContent = memo(({ imageId }: { imageId: string }) => {
  ...
  const imagePromiseRef = useRef<ReturnType<typeof getImageById> | null>(null)

  if (!imagePromiseRef.current) {
    imagePromiseRef.current = getImageById(imageId)
  }

  const image = use(imagePromiseRef.current)
...
ImageDetailContent.displayName = 'ImageDetailContent'
